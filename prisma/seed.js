...
async function main() {
  ...
  const generalDentistry = await prisma.specialty.findFirst({
    where: { name: "General Dentistry" },
  });
  const orthodontics = await prisma.specialty.findFirst({
    where: { name: "Orthodontics" },
  });
  const oralSurgery = await prisma.specialty.findFirst({
    where: { name: "Oral Surgery" },
  });
  ...
  const paymentStatusPaid = paymentStatuses.find(
    (status) => status.status === "PAID"
  );
  const paymentStatusPending = paymentStatuses.find(
